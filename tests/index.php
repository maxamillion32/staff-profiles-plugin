...
$apa_obj = new sp_apa_publication();
$apa_obj->set_display($display);
foreach ($pub_types as $type) {
	...
	foreach ( $data as $pub ) {
		if ( $pub['publicationtype'] == $type ) {
			...
			printf('<h4>Symplectic formatted entry</h4><div class="publication-formatted">%s</div>', html_entity_decode($pub["displaycitationhtml"]) );
			...
			$apa_obj->set_publication($pub);
			printf('<h4>APA formatted entry</h4><div class="publication-formatted"><ul class="publications apa">%s</ul></div>', $apa_obj->get_formatted_publication());
			$count++;
		}
	}
}
